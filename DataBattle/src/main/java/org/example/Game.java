...
public class Game {
    ...
    public void playRound() {
        ...
        Player bestDefender = null; //Ele só existe se tiver apenas um atacante, nesse caso ele é o cara que vai ter dano reduzido
        ...
        for(Player player : players){
            ...
            } else if (attackers.size() == 1 && bestDefender == null) { //Se só tiver um numero maior na rodada, o segundo maior vira o bestDefender
                bestDefender = player;
            }
        }
        ...
        atacarJogadores(players, attackers, fuckedPlayers ,bestDefender, baseDamage); //Separei a lógica para fica mais legível

        players.removeIf(player -> player.getHealth() <=0); //Remove da lista os jogadores que foram eliminados

    }

    private void atacarJogadores(List<Player> players, List<Player> attackers, List<Player> fuckedPlayers ,Player bestDefender, int baseDamage) {
        for(Player player : players){
            if(!attackers.contains(player) && !player.equals(bestDefender)){ //Ataca todos os jogadores, exceto o bestDefender e os atacantes;
        ...
        if(attackers.size()==1 && bestDefender!=null){ //Nesse caso, como só tem um atacante, o bestDefender defende metade do dano
        ...
        if(attackers.size() >1){ //Se houver mais de um atacante, então eles mesmmos serão os bestDefenders
            int reducedDamage = baseDamage / 2;
